...
bgimg2 = pygame.image.load("bg2.jpg")
...
def welcome():
    ...
    while not exit_game:
        ...
        text_screen("Welcome to Snakes", white, 180, 200)
        text_screen("Press Space Bar To Play", white, 160, 230)
    ...
    while not exit_game:
        if game_over:
            with open("hiscore.txt", "w") as f:
                f.write(str(hiscore))
            gameWindow.fill(red)
            text_screen("Game Over! Press Enter To Continue", white, 80, 220)
            ...
            pygame.draw.rect(gameWindow, (0,255,100), [food_x, food_y, snake_size, snake_size])
